...
H_1 <- PointMassPrior(get_tau_ZSquared(0.4), 1)
...
H_0 <- PointMassPrior(0, 1)
H_1 <- PointMassPrior(get_tau_ZSquared(.4, 1), 1)
...
initial_design <- get_initial_design(
    ...
    beta  = .2,
    type_design  = "two-stage",
    dist  = ZSquared(two_armed = TRUE),
    order = 7L
)
# initial_design@c1e <- initial_design@c1e * 1.2
# initial_design@c2_pivots <- initial_design@c2_pivots * 1.2
# initial_design@n2_pivots <- initial_design@n2_pivots * 2
evaluate(ess, initial_design)
evaluate(power, initial_design)
evaluate(toer, initial_design)
